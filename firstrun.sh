#!/usr/local/bin/bash
  ...
  # Shutdown any previously started servers
  if [ -e ${HOME}/tomcat/webapps/opensso ] ; then
     echo "Stopping Tomcat..."
     sudo /sbin/service tomcat stop
  fi
  if [ -e ${HOME}/OpenDJ ] ; then
  ...
  MEMSIZE=$((`awk '/MemTotal:/ {print $2 }' /proc/meminfo` / 1024))
  ...
  echo "How many test credentials do you wish to create?"
  read -p " => " -e -i "100" NUMUSERS

  echo
  echo "Creating local trusted SSL keystore..."
  rm -rf ${HOME}/SSL
  mkdir ${HOME}/SSL
  cp ${JAVA_HOME}/lib/security/cacerts  ${HOME}/SSL/jssecacerts
  ...
  if [ "${PROTOCOL}" = "https" ] ; then
     ...
     openssl x509 -req -days 2190 -sha256 -in ${HOME}/SSL/tomcat.csr -signkey ${HOME}/SSL/private.key \
  ...
  if [ -e ${HOME}/OpenDJ ] ; then
     echo "Removing old OpenDJ configuration..."
     rm -rf ${HOME}/OpenDJ
  fi
  ...
  unzip -q -o -d ${HOME} ${HOME}/tools/OpenDJ-2.5.0-Xpress1.zip
  mv ${HOME}/OpenDJ-2.5.0-Xpress1 ${HOME}/OpenDJ
  ${HOME}/OpenDJ/setup -i -O -n -p 1389 -b "dc=gc,dc=ca" -d ${NUMUSERS} -h ${FQDN} -w SAMLTest1
  DJ_HEAP=$((${MEMSIZE}/3))
  sed -i ${HOME}/OpenDJ/config/java.properties -e "s^start-ds.java-args.*^start-ds.java-args=-server -Xmx${DJ_HEAP}m^"
  ${HOME}/OpenDJ/bin/dsjavaproperties -Q

  echo "Starting OpenDJ..."
  sudo /sbin/chkconfig --levels 3 opendj on
  ...
  TC_HEAP=$((${MEMSIZE}*2/3))
  sed -i ${HOME}/tomcat/bin/setenv.sh -e "s^-Xmx[0-9]*[mk]^-Xmx${TC_HEAP}m^"
  touch ${HOME}/tomcat/logs/catalina-daemon.out
  ...
  echo -e "\
     ...
     USERSTORE_HOST=${FQDN}\n\
     USERSTORE_PORT=1389\n\
     USERSTORE_SUFFIX=dc=gc,dc=ca\n\
     USERSTORE_MGRDN=cn=Directory Manager\n\
     USERSTORE_PASSWD=SAMLTest1\n\
  " > /tmp/amconfig.txt
  ...
  ${JAVA_HOME}/bin/java -jar ${HOME}/tools/Signer.jar \
      ...
      ${HOME}/ICM/keystore.jks idpsimpub-signing ${SIGALG}
  ...
  echo -e "\
     update-server-cfg -s ${PROTOCOL}://${FQDN}:${PORT}/opensso \
        -a com.iplanet.am.cookie.name=IDPSimulator\n\
     update-server-cfg -s ${PROTOCOL}://${FQDN}:${PORT}/opensso \
        -a com.iplanet.am.cookie.encode=true\n\
     update-server-cfg -s ${PROTOCOL}://${FQDN}:${PORT}/opensso \
        -a com.iplanet.am.cookie.secure=${SECURE_COOKIE}\n\
     update-server-cfg -s ${PROTOCOL}://${FQDN}:${PORT}/opensso \
  ...
  if [ "${SIGALG}" = "SHA256" ] ; then 
   ...
   ## Added by SF ##
   sudo sed -i -e "s^##DOMAIN_NAME##^${DOMAINNAME}^g" ${HOME}/tomcat/webapps/opensso/config/auth/default/Login.jsp
   sudo sed -e "s^##DOMAIN_NAME##^${DOMAINNAME}^g" ${HOME}/templates/index-template.jsp > ${HOME}/tomcat/webapps/ROOT/index.jsp

   echo "Rebuilding Indexes..."
   sudo /sbin/service tomcat stop
   sudo /sbin/service opendj stop
   ${HOME}/OpenDJ/bin/rebuild-index -b "dc=gc,dc=ca" --rebuildDegraded
   sudo /sbin/service opendj start
   sudo /sbin/service tomcat start
